"""
Court Data Processing Application v2.0

Implemented ttkbootstrap in existing tkinter code to add styling
minor bug fixes
...
data4_pattern = r"CLS:(\d)\s+P:(\w+)?\s+L:(\w+)?\s+JUDGMENT:(\w+)?"
...
class CourtApp:
    ...
    def __init__(self, root):
        ...
        # Title Label
        tb.Label(root,  text="Select Court Text Files (Up to 3)", font=("Arial", 12, "bold")).grid(row=0, column=0, columnspan=3, pady=10)

        # File 1
        tb.Label(root,  text="File 1:").grid(row=1, column=0, padx=10, sticky="NESW")
        self.entry1 = tb.Entry(root, width=50)
        self.entry1.grid(row=1, column=1, padx=5)
        tb.Button(root, text="Browse", command=lambda: self.browse_file(self.entry1)).grid(row=1, column=2, padx=5)

        # File 2 
        tb.Label(root,  text="File 2:").grid(row=2, column=0, padx=10, sticky="NESW")
        self.entry2 = tb.Entry(root, width=50)
        self.entry2.grid(row=2, column=1, padx=5)
        tb.Button(root, text="Browse", command=lambda: self.browse_file(self.entry2)).grid(row=2, column=2, padx=5)

        # File 3
        tb.Label(root,  text="File 3:").grid(row=3, column=0, padx=10, sticky="NESW")
        self.entry3 = tb.Entry(root, width=50)
        self.entry3.grid(row=3, column=1, padx=5)
        tb.Button(root, text="Browse", command=lambda: self.browse_file(self.entry3)).grid(row=3, column=2, padx=5)

        # Process Button
        tb.Button(root, text="PROCESS DATA", command=self.process_data).grid(row=4, column=0, columnspan=3, pady=20)
    ...
    def process_data(self):
        ...
        # Remove old output unless it has been renamed
        try:    
            if os.path.exists('Court_Output.xlsx'):
                os.remove('Court_Output.xlsx')
        except PermissionError:
            messagebox.showerror("Error", "Please close or rename 'Court_Output.xlsx' before running.")
            return
        ...
        for textfile in files:
            ...
            for line in content:
                ...
                if data3_match:
                    new_charge = data3_match.group(1)
                ...
                if data4_match and master:
                    master[-1]['CLS'] = data4_match.group(1)
                    master[-1]['P'] = data4_match.group(2)
                    master[-1]['L'] = data4_match.group(3)
                    master[-1]['Judgment'] = data4_match.group(4)
        ...
        if master:
            ...
            #Converting to table using xlsxwriter
            df.to_excel(writer, sheet_name='DataAsTable', index=False)
            workbook = writer.book
            worksheet = writer.sheets['DataAsTable']
            (max_row,max_col) = df.shape
            header_format = workbook.add_format({'bold': True, 'bg_color': '#D9D9D9'})
            worksheet.add_table(0,0,max_row,max_col,{'columns': [{'header': col,'header_format':header_format} for col in df.columns]})
            ...
            # --- AUTO OPEN EXCEL ---
            launch_excel = messagebox.askyesno("Processing Complete","Would you like to open Excel?",icon='question')
            if launch_excel:
                try:
